 ...
 * @param {Array} cartItems Items del carrito con propiedades: height, width, length, weight, quantity
...
const consolidateCartItems = (cartItems) => {
  ...
  // La altura final considera el apilamiento de productos
  const finalHeight = maxHeight * totalUnits;

  return {
    ubl: 0, // Campo requerido por Coordinadora (usado en generación de guía)
    alto: finalHeight,
    ancho: maxWidth,
    largo: maxLength,
    peso: totalWeight,
    unidades: 1, // UN SOLO PAQUETE consolidado
    referencia: '', // Campo opcional para referencia interna
  };
};
...
export const getCoordinadoraQuote = async (destinationCityCode, cartItems) => {
  ...
  // Consolidar todos los productos en un solo paquete
  ...
  const args = {
    p: {
      ...
      div: '00',
      cuenta: 3,           // Código de cuenta (usualmente 3 o 1)
      producto: 0,         // <-- ESTA ES LA SOLUCIÓN (0 = Paquetería)
      nivel_servicio: 1,   // Nivel de servicio (1 = Estándar)
...
export const generateCoordinadoraGuide = async (orderData, orderId, userId) => {
  ...
  // CORRECCIÓN: Consolidar todos los productos en UN SOLO PAQUETE
  // Esto evita que se generen múltiples rótulos (uno por producto)
  ...
  const args = {
    p: {
      ...
      detalle: {
        // CRÍTICO: Enviar UN SOLO ITEM consolidado con todos los productos
